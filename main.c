...
int main(int argc, char *argv[]) {
    ...
    long RandSeed;
    int i, j, numCustomers, numChairs;
    ...
    // barber thread.
    allDone = 1;
    up(&barberPillow);  // Wake the barber so he will exit.
...
void *barber(void *number) {
	...
	printf("The %d barber is sleeping\n", num);
	down(&barberPillow);
	...
	if (!allDone) {
	    ...
	    // customer's hair.
	    printf("The %d barber is cutting hair\n", num);
	    randwait(3);
	    printf("The %d barber has finished cutting hair.\n", num);
